...
function ewd_us_create_watermarked_image( $image_url ) {
	...
	$image_file_path = $plugin_upload_path . basename( $image_url );
	...
	$margin_right = 10;
	$margin_bottom = $height - ( $width * $aspect_ratio_fraction ) + 10;
	$sx = imagesx( $scaled_stamp );
	$sy = imagesy( $scaled_stamp );
	 
	imagecopy( $image, $scaled_stamp, imagesx( $image ) - $sx - $margin_right, imagesy( $image ) - $sy - $margin_bottom, 0, 0, imagesx( $scaled_stamp ), imagesy( $scaled_stamp ) );
